...
public class PathEditState {
    ...
    private boolean  appendMode    = false;   // true = clicks add waypoints

    public void beginEdit(PathData path) {
        source = path; active = true;
        hoveredPoint = -1; selectedPoint = -1;
        dragging = false; appendMode = false;
        copyFrom(path);
    }

    public void endEdit() {
        active = false; source = null; wx = wz = null;
        hoveredPoint = selectedPoint = -1;
        dragging = false; appendMode = false;
    }
    ...
    private void copyFrom(PathData path) {
        ...
        for (int i = 0; i < n; i++) {
            wx[i] = path.points.get(i)[0];
            wz[i] = path.points.get(i)[1];
        }
    }
    ...
    public void insertPointAfter(int edgeIndex) {
        ...
        float mx = (wx[edgeIndex] + wx[j]) / 2f;
        float mz = (wz[edgeIndex] + wz[j]) / 2f;
        wx = insertAt(wx, edgeIndex + 1, mx);
        wz = insertAt(wz, edgeIndex + 1, mz);
    }
    ...
    public void movePoint(int idx, float x, float z) {
        if (idx < 0 || wx == null || idx >= wx.length) return;
        wx[idx] = x; wz[idx] = z;
    }
    ...
    private float[] insertAt(float[] arr, int pos, float v) {
        ...
        n[pos] = v;
        System.arraycopy(arr, pos, n, pos + 1, arr.length - pos);
        return n;
    }

    private float[] removeAt(float[] arr, int idx) {
        float[] n = new float[arr.length - 1];
        for (int i = 0, k = 0; i < arr.length; i++) { if (i != idx) n[k++] = arr[i]; }
        return n;
    }
}