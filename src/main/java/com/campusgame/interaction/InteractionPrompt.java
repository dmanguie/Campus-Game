...
/**
 * Renders the "[E] to enter Library" prompt.
 *
 * Two elements:
 *   1. A pulsing dot + line above the door in world-space
 *   2. A centred prompt box at the bottom of the screen
 *
 * The "[E]" substring in getPromptText() is coloured yellow automatically.
 */
public class InteractionPrompt {
    ...
    private void drawPromptBox(Graphics2D g, String prompt,
        ...
        // Coloured text — "[E]" in yellow, rest in grey
        ...
        if (eIdx >= 0) {
            ...
            int bodyA = (int)(breathe * 220);
            int keyA  = (int)(breathe * 255);

            if (!before.isEmpty()) {
                g.setColor(new Color(BODY_COL.getRed(), BODY_COL.getGreen(), BODY_COL.getBlue(), bodyA));
                g.drawString(before, tx, ty);
                tx += fm.stringWidth(before);
            }
            g.setColor(new Color(KEY_COL.getRed(), KEY_COL.getGreen(), KEY_COL.getBlue(), keyA));
            ...
            g.setColor(new Color(BODY_COL.getRed(), BODY_COL.getGreen(), BODY_COL.getBlue(), (int)(breathe * 220)));
